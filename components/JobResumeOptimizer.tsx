...
import { FileText, Upload, ChevronRight, Copy, Check } from "lucide-react";
...
function formatResumeData(data: ResumeData | unknown): string {
  ...
  if (resumeData.contact) {
    ...
    if (resumeData.contact.email)
      formatted += `Email: ${resumeData.contact.email}\n`;
    if (resumeData.contact.phone)
      formatted += `Phone: ${resumeData.contact.phone}\n`;
    if (resumeData.contact.linkedin)
      formatted += `LinkedIn: ${resumeData.contact.linkedin}\n`;
    if (resumeData.contact.address)
      formatted += `Address: ${resumeData.contact.address}\n`;
    if (resumeData.contact.portfolio)
      formatted += `Portfolio: ${resumeData.contact.portfolio}\n`;
    formatted += "\n";
  }
...
export default function JobResumeOptimizer() {
  ...
  }>({ resume2: null, jobDescription: null });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (useFileUpload2) {
        if (!files.resume2) {
          setError(
            "Please upload a resume file for the job-optimization flow."
          );
          setLoading(false);
          return;
        }
        ...
        if (!resume2Text.trim()) {
          setError(
            "Please paste resume text or choose upload for the job-optimization flow."
          );
          setLoading(false);
          return;
        }
  ...
  return (
    <motion.div
      ...
      } rounded-2xl shadow-xl border p-6 sm:p-8 hover:shadow-2xl transition-all duration-300 backdrop-blur-sm`}
      ...
      <div className="flex items-start space-x-3 sm:space-x-4">
        ...
        <div className="flex-1">
          ...
          <p
            ...
            } mb-4 sm:mb-6 text-base sm:text-lg`}
          >
            Upload your resume and a job description. Get a version tailored for
            the JD with relevant skills highlighted and keywords optimized.
            NOTE: This option uses free AI service, which might use your data
            for training purposes. Avoid including sensitive personal
            information.
          </p>
              ...
              {useFileUpload2 ? (
                <div className="relative">
                  <input
                    ...
                    type="file"
                    accept=".pdf,.docx,.txt"
                    onChange={handleFileChange("resume2")}
                    className="hidden"
                  />
                  <label
                    ...
                    className={`flex items-center justify-center w-full px-4 py-3 border-2 border-dashed ${
                      darkMode
                        ? "border-emerald-500/30 hover:border-emerald-400 hover:bg-emerald-500/10"
                  ...
                  >
                    ...
                    >
                      {files.resume2
                        ? files.resume2.name
                        : "Choose resume file or drag and drop"}
                    </span>
                  </label>
                </div>
              ) : (
                <div>
                  <textarea
                    ...
                    className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-emerald-500 transition-all duration-300 resize-none ${
                      darkMode
                        ? "bg-emerald-950/30 border-emerald-500/30 text-emerald-100 placeholder-emerald-400/60"
                  ...
                  >
                    ...
                    >
                      {files.jobDescription
                        ...
                        : "Choose job description file or drag and drop"}
                    </span>
                  </label>
                </div>
              ) : (
          ...
          <div className="flex flex-col gap-3">
            <motion.button
              ...
              className={`w-full px-6 sm:px-8 py-4 sm:py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center space-x-2 group ${
                loading ||
                !!resultText ||
                (useFileUpload2 ? !files.resume2 : !resume2Text.trim()) ||
                (useFileUpload3
                  ...
                  : darkMode
                  ? "bg-gradient-to-r from-emerald-500 to-indigo-600 hover:from-emerald-600 hover:to-indigo-700 hover:shadow-2xl"
                  : "bg-gradient-to-r from-emerald-600 to-indigo-600 hover:from-emerald-700 hover:to-indigo-700 hover:shadow-2xl"
              } text-white`}
              whileHover={{
                scale:
                  loading ||
            ...
            {resultText && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <motion.button
                  className={`px-4 sm:px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center space-x-2 group ${
                    darkMode
                      ? "bg-gradient-to-r from-emerald-500 to-indigo-600 hover:from-emerald-600 hover:to-indigo-700 hover:shadow-2xl"
                      : "bg-gradient-to-r from-emerald-600 to-indigo-600 hover:from-emerald-700 hover:to-indigo-700 hover:shadow-2xl"
                  } text-white`}
                ...
                <motion.button
                  ...
                  className={`px-4 sm:px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center space-x-2 group ${
                    darkMode
                      ? "bg-emerald-500/20 text-emerald-300 border-2 border-emerald-500/30 hover:bg-emerald-500/30 hover:border-emerald-400/50"
                      : "bg-emerald-100 text-emerald-700 border-2 border-emerald-200 hover:bg-emerald-200 hover:border-emerald-300"
                  } backdrop-blur-sm`}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                >
          ...
          {resultText && (
            <div className="relative mt-4">
              ...
              <pre
                className={`whitespace-pre-wrap p-4 rounded-xl border ${
                  darkMode
